...
const DAY_LABEL = "ראשון 23.8";
// Try 17:00 first; fall back to 16:00 if full or not found
const TIMES = ["17:00", "16:00"];
...
console.log(`One-time enrollment — ${DAY_LABEL}, trying: ${TIMES.join(", ")}`);
...
for (const time of TIMES) {
    ...
    if (isEnrolled(cls)) {
        ...
        if (!DRY_RUN && ALERTZY_KEY) {
            await sendPushNotification(ALERTZY_KEY, "✅ כבר רשומה", `כבר רשומה ל-${cls.box_categories.name} ${time} (${DAY_LABEL})`);
        }
        process.exit(0);
    }
...
if (!DRY_RUN && ALERTZY_KEY) {
    await sendPushNotification(ALERTZY_KEY, "⏳ כל השיעורים מלאים", `כל השיעורים מלאים (17:00, 16:00) — ${DAY_LABEL}\nהירשמי ידנית להמתנה`);
}
process.exit(0);
